...
static void initialize_UART_0_IRQ_Rx(uint rxPin, uint txPin) {
    ...
    UART_0_Wrapper.setHandlerForIRQ(UART_0_Rx_IRQ_Handler);

    assert(UART_0_Wrapper.enableRxIRQ(true) && "Error while UART 0 Rx IRQ enable");
}

static void UART_0_Rx_IRQ_Handler() {
    while (UART_0_Wrapper.isReadable()) {
        Rx_Buffer.insert(UART_0_Wrapper.read());
    }
}
